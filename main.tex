...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{figures/CTOexample_cmyk.jpeg}
...
\begin{figure*}[ht]
\centering
\includegraphics[width=0.9\textwidth]{figures/candidategenerationcolor_cmyk.jpeg} 
...
%To compare the weakly annotated dataset and the distant NER model on the ``Intervention'' recognition on the PICO benchmark with four strong baselines trained on EBM-PICO dataset from the previous studies were used.
\begin{itemize}
    ...
    %\item \textbf{b5:} Used pretrained BERT embeddings followed by a linear decoder.
\end{itemize}
%
...
\begin{table*}[hbt!]
    ...
    \begin{tabular}{clccc|ccc}
        ...
        \Xhline{1pt}
    \end{tabular}
    \caption{Macro-averaged performance metrics for the NER models trained on weakly annotated DISTANT-CTO alone \textit{vs.} in combination to the strongly annotated EBM-PICO on the two described benchmarks (EBM-PICO evaluation corpus and the Physio corpus). The results are compared with the baseline (b5) that used only EBM-PICO for training and results from the previous studies (b1-b4).} %Henning: the caption needs to be added %Anjani addressed
...
%
%The micro-averaged performance metrics (Precision, Recall and F1) on token-level ``Intervention'' entity across three experimental repetitions are provided in the table \ref{tab:results_modeltraining}.
...
%Henning: this phrase should be reformulated, as it is not really clear to me. %Anjani addressed
...
%Henning: also this phrase is not fully clear to me.
...
%Henning: this phrase needs to be changed, what is meant here?
...
%further adding to the false negatives.
...
%Henning: such bulleted lists need to be introduced and I do not see the link with the text before
